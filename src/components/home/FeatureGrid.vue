<template>
  <section class="feature-grid">
    <div class="feature-grid__header">
      <h2 class="feature-grid__title">功能亮点</h2>
      <p class="feature-grid__subtitle">一站式简历编辑解决方案</p>
    </div>
    <div class="feature-grid__items">
      <div
        v-for="(feature, idx) in features"
        :key="feature.title"
        class="feature-card"
        :style="{ animationDelay: `${idx * 0.08}s` }"
      >
        <div class="feature-card__icon">
          <Icon :icon="feature.icon" :width="32" />
        </div>
        <div class="feature-card__title">{{ feature.title }}</div>
        <div class="feature-card__desc">{{ feature.desc }}</div>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
import { Icon } from '@iconify/vue'

const features = [
  { icon: 'mdi:palette-outline', title: '多模板支持', desc: '8 款专业模板，一键切换，风格多样' },
  { icon: 'mdi:auto-fix', title: 'AI 智能润色', desc: '7 种 AI 文本操作，让表达更专业' },
  { icon: 'mdi:clipboard-check-outline', title: '简历评估', desc: 'AI 全方位评估打分，精准定位不足' },
  { icon: 'mdi:text-search', title: 'JD 匹配分析', desc: '智能关键词扫描，提升投递匹配度' },
  { icon: 'mdi:creation', title: '一键优化', desc: 'AI 系统性润色整份简历，全面提升' },
  { icon: 'mdi:account-tie', title: '面试准备', desc: '生成面试题和复习要点，从容应战' },
  { icon: 'mdi:file-import-outline', title: 'AI 智能导入', desc: '上传 PDF/Word/Markdown，AI 解析为结构化简历' },
  { icon: 'mdi:comment-question-outline', title: 'AI 智能咨询', desc: '常驻侧栏多轮对话，文件图片上传，左右切换' },
  { icon: 'mdi:cat', title: 'Lottie 桌宠', desc: 'v仔桌宠伴你编辑，自定义上传图片/Lottie，拖拽吸附' },
  { icon: 'mdi:delete-restore', title: '回收站保护', desc: '删除内容可恢复，30 天暂存防误删' },
  { icon: 'mdi:eye-outline', title: '实时预览', desc: '所见即所得编辑体验，边改边看' },
  { icon: 'mdi:download', title: '多格式导出', desc: 'PDF / 图片 / DOCX / JSON 一键导出' },
  { icon: 'mdi:moon-waning-crescent', title: '深色模式', desc: '护眼深色主题，夜间编辑更舒适' },
  { icon: 'mdi:folder-sync-outline', title: '目录模式同步', desc: '绑定本地文件夹，自动双向同步简历数据' },
  { icon: 'mdi:shield-check-outline', title: '数据安全', desc: '本地存储，隐私无忧，无需注册' },
]
</script>

<style lang="scss" scoped>
.feature-grid {
  padding: $spacing-3xl 0;
}

.feature-grid__header {
  text-align: center;
  margin-bottom: $spacing-2xl;
}

.feature-grid__title {
  font-size: $font-size-3xl;
  font-weight: 700;
  color: $text-primary;
  margin-bottom: $spacing-sm;
}

.feature-grid__subtitle {
  font-size: $font-size-lg;
  color: $text-secondary;
}

.feature-grid__items {
  display: grid;
  grid-template-columns: repeat(4, 1fr);
  gap: $spacing-lg;

  @include tablet {
    grid-template-columns: repeat(2, 1fr);
  }
}

// Apple 风格：卡片无 glass 效果，无阴影
.feature-card {
  @include card-container;
  display: flex;
  flex-direction: column;
  align-items: center;
  text-align: center;
  padding: $spacing-xl $spacing-lg;
  animation: slide-up 0.6s ease both;
}

// Apple 风格：图标背景使用 hover 色
.feature-card__icon {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 56px;
  height: 56px;
  border-radius: $radius-lg;
  background: var(--bg-glass-hover);
  color: $primary-color;
  margin-bottom: $spacing-md;
}

.feature-card__title {
  font-size: $font-size-md;
  font-weight: $font-weight-semibold;
  color: $text-primary;
  margin-bottom: $spacing-xs;
}

.feature-card__desc {
  font-size: $font-size-sm;
  color: $text-secondary;
  line-height: 1.5;
}

@keyframes slide-up {
  from {
    opacity: 0;
    transform: translateY(20px);
  }
  to {
    opacity: 1;
    transform: translateY(0);
  }
}
</style>
