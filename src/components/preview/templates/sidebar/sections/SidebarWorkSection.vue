<template>
  <section class="sidebar__section" data-export-cv :data-section="sectionId" @click="emit('click-section', sectionId)">
    ...
    <div v-if="ctx.getVisibleWorkItems.value.length">
      <div v-for="item in ctx.getVisibleWorkItems.value" :key="item.id" class="main__entry" data-export-cv :data-item-id="item.id" @click.stop="emit('click-section', sectionId, item.id)">
