<template>
  <section class="sidebar__section" data-export-cv :data-section="sectionId" @click="emit('click-section', sectionId)">
    ...
    <div v-for="item in ctx.getVisibleProjectItems.value" :key="item.id" class="main__entry" data-export-cv :data-item-id="item.id" @click.stop="emit('click-section', sectionId, item.id)">
      ...
      <div v-if="item.technologies?.length" class="main__entry-tags" data-export-flex-child>
        <span v-for="(tech, idx) in item.technologies" :key="idx" class="main__tech-tag" data-export-flex-child data-export-nowrap>{{ tech }}</span>
      </div>
    </div>
  </section>
</template>
