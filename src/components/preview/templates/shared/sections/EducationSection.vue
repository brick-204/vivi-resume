<template>
  <section class="resume__section" data-export-cv data-section="education" @click="emit('click-section', 'education')">
    <h2 class="section__title">
      ...
      {{ ctx.getSectionTitle(ctx.resume, 'education') }}
    </h2>
    <div v-for="item in ctx.getVisibleEducationItems.value" :key="item.id" class="entry" data-export-cv :data-item-id="item.id" @click.stop="emit('click-section', 'education', item.id)">
