<template>
  <section class="resume__section" data-export-cv data-section="work" @click="emit('click-section', 'work')">
    ...
    <div v-for="item in ctx.getVisibleWorkItems.value" :key="item.id" class="entry" data-export-cv :data-item-id="item.id" @click.stop="emit('click-section', 'work', item.id)">
