...
- **富文本编辑** - 基于 Tiptap 的富文本编辑器，支持加粗、斜体、列表等；求职手账内支持插入图片（Worker 压缩 WebP）与待办清单
...
- **记事本 / 笔记二级结构** - 记事本作为目录容器分类整理，笔记可在记事本内或根级；拖拽笔记到记事本上即可收入，记事本内可一键移出
- **富文本 + 图片 + 待办** - 基于 Tiptap 的富文本编辑，支持工具栏/粘贴/拖拽插入图片（经 Web Worker 压缩为 WebP）、内联待办清单（TaskList/TaskItem）
- **关联面试与简历** - 每条笔记可关联多条面试和多份简历，卡片上以徽章显示关联数量
- **搜索与排序** - 按标题或内容搜索，按更新时间/创建时间排序，升降序可切
- **自动标题** - 退出编辑时标题为空则自动取正文第一行前 6 字，标题一旦填入永不自动覆盖
- **AI 去简历化** - 手账内 AI 操作走独立 `journal` 场景 prompt，不注入简历上下文
- **回收站与双后端** - 删除进回收站软删除，保留期到期自动清理；IndexedDB / 目录模式同步，与简历同骨架
...
├── components/
│   ├── common/            # 公共组件（AppHeader、RichTextEditor、ResizeHandle、ResizableImageView）
...
│   │   ├── JournalPanel.vue       # 求职手账面板（记事本/笔记二级 + 搜索排序 + 回收站）
...
│   ├── journalStore.ts     # 求职手账管理（记事本/笔记 CRUD + 回收站 + 双后端持久化）
...
│   ├── journal.ts         # 求职手账类型（JournalEntry + 记事本/笔记工厂 + 标题派生）
...
Dashboard 侧边「求职手账」入口，独立于简历的笔记管理面板，用于记录求职心得、面试复盘、准备清单：

- **记事本与笔记** - 新建记事本作为分类目录（如「面试复盘」「公司调研」），新建笔记记录具体内容；笔记可放在根级或某个记事本内
- **拖拽收入** - 拖动笔记卡片到记事本上即可将其收入该记事本；记事本内可点「移出」回到根级
- **富文本编辑** - 支持加粗、列表等基础格式，可插入图片（粘贴/拖拽/工具栏，自动压缩为 WebP）和待办清单
- **关联面试与简历** - 编辑笔记时可选关联多条面试和多份简历，卡片上以徽章显示关联数量，便于交叉查阅
- **搜索与排序** - 顶部搜索框按标题或内容过滤，排序可按更新时间/创建时间切换升降序
- **自动标题** - 退出编辑时若标题为空，自动取正文第一行前 6 字作为标题；标题一旦手动填写则不再自动覆盖
- **AI 辅助** - 手账内的 AI 文本操作（润色/扩展等）走独立场景，不注入简历上下文，聚焦笔记本身
- **回收站** - 删除的记事本/笔记进入回收站，保留期内可恢复，到期自动清理
